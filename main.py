...
class SplunkLogAnalysis:
    ...
    def generate_process_report_batch(self, user_input: str, batch_size: int = 5, 
                                    max_results: int = 10000, output_file: str = None) -> pd.DataFrame:
        ...
        if output_file is None:
            ...
            output_file = f"{safe_name}_splunk_analysis.csv"
...
def main():
    """Main function to run the Splunk log analysis with improved error handling"""
    ...
    print("="*80)
    print("SPLUNK LOG ANALYSIS SYSTEM")
    print("="*80)
    
    while True:
        try:
            print("\nOptions:")
            print("1. Search for specific process")           
            print("2. Run custom SPL query")
            print("3. Quit")
            
            choice = input("\nEnter your choice: ").strip()

            if choice == '1':
                process_name = input("Enter process name to search for: ").strip()
                
                if process_name:
                    df = analyzer.generate_process_report_batch(process_name, 5)
                ...
                if spl_query:
                    try:
                        max_results = int(input("Max results (default: 100): ").strip() or "100")
                    except ValueError:
                        max_results = 100
                    
                    results = analyzer.run_custom_query(spl_query, max_results)
                    ...
                    if results:
                        ...
                        for i, result in enumerate(results[:3]):
                            print(f"\nResult {i+1}:")
                            important_fields = ['_time', 'application', 'process_name', 'message', 'log_level', 'correlation_id']
